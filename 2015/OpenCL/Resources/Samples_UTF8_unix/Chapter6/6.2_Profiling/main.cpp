...
static int printProfilingInfo(cl_event event);
static int printCommandStatus(cl_event event);
...
main()
{
    ...
    cl_event kernel_exec_event = (cl_event)0;
    ...
    // コマンドステータスの取得と表示
    if (printCommandStatus(kernel_exec_event) != 0) {
        return 9;
    }
    ...
    // カーネル実行コマンドプロファイリング情報の取得と表示
    if (printProfilingInfo(kernel_exec_event) != 0) {
        return 10;
    }
...
/*-------------------------------------------------------------------------
 * 引数で与えられたイベントに関するプロファイリング情報を表示する
 */
static int
printProfilingInfo(cl_event event)
{
    ...
    status = clGetEventProfilingInfo(event,
                                     ...
                                     NULL);
    if (status != CL_SUCCESS) {
        fprintf(stderr, "clGetEventProfilingInfo.\n");
        printError(status);
        return 10;
    }
    status = clGetEventProfilingInfo(event,
    ...
    // 結果の表示
    printf("\n");
    printf("CL_PROFILING_COMMAND_QUEUED: %ld\n", (long)time_queued);
    printf("CL_PROFILING_COMMAND_SUBMIT: %ld (+%ld (ns))\n",
           (long)time_submit, (long)(time_submit - time_queued));
    printf("CL_PROFILING_COMMAND_START : %ld (+%ld (ns))\n",
           (long)time_start, (long)(time_start - time_submit));
    printf("CL_PROFILING_COMMAND_END   : %ld (+%ld (ns))\n",
           (long)time_end, (long)(time_end - time_start));
    
    return 0;
}

/*-------------------------------------------------------------------------
 * 引数で与えられたイベントに関連付けられているコマンドのステータスを表示する
 */
static int
printCommandStatus(cl_event event)
{
    cl_int status;
    cl_int exec_status;
    ...
    switch (exec_status) {
        ...
        default:
            if (exec_status < 0) {
                // 負の値はエラーコード
                printf("Error code: %d", exec_status);
            } else {
